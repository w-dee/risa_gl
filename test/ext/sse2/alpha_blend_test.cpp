...
class alpha_blend_operator_test : public CppUnit::TestFixture
{
	...
	CPPUNIT_TEST(alpha_blend_save_source_alpha_test);
	CPPUNIT_TEST(alpha_blend_save_destination_alpha_test);
...
public:
	...
	void alpha_blend_save_source_alpha_test()
	{
		using namespace risa_gl;
		 ...
		 * = (0.25, 0.25, 0.25, 0.5)
		 */
		operators::alpha_blend_save_source_alpha_operator<pixel> oper;
		oper(adapter_type::to_alignment(src.begin()),
			 adapter_type::to_alignment(dest.begin()),
			 adapter_type::to_alignment(result.begin()));
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			CPPUNIT_ASSERT(range<int>(result(i,0).get_green(), 64, 1)); 
			CPPUNIT_ASSERT(range<int>(result(i,0).get_blue(), 64, 1));
			CPPUNIT_ASSERT(range<int>(result(i,0).get_alpha(), 128, 1));
		}
	...
	void alpha_blend_save_destination_alpha_test()
	{
		using namespace risa_gl;
		 ...
		 * = (0.25, 0.25, 0.25, 0.5)
		 */
		operators::alpha_blend_save_destination_alpha_operator<pixel> oper;
		oper(adapter_type::to_alignment(src.begin()),
			 adapter_type::to_alignment(dest.begin()),
			 adapter_type::to_alignment(result.begin()));
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			CPPUNIT_ASSERT(range<int>(result(i,0).get_green(), 64, 1)); 
			CPPUNIT_ASSERT(range<int>(result(i,0).get_blue(), 64, 1));
			CPPUNIT_ASSERT(range<int>(result(i,0).get_alpha(), 256, 1));
		}
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			CPPUNIT_ASSERT(range<int>(result(i,0).get_green(), 191, 1)); 
			CPPUNIT_ASSERT(range<int>(result(i,0).get_blue(), 191, 1));
			CPPUNIT_ASSERT(range<int>(result(i,0).get_alpha(), 1, 1));
		}
	}
};
