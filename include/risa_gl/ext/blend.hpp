		...
		namespace sse2
		{
			struct odd_mask
			{
					...
					const aligned_wideword_type& source) const
				{
					...
					const aligned_wideword_type mask = _mm_set1_epi16(0x00ff);
			...
			struct even_mask
			{
					...
					const aligned_wideword_type& source) const
				{
					...
					const aligned_wideword_type mask = _mm_set1_epi16(0xff00);
					return _mm_and_si128(source, mask);
				}
			};
								...
								result_itor_t result) const
				{
					...
					aligned_wideword_type res_pixel =
						odd_shifter(
							compute_factor(
								divisor_factor(
									multiplier(odd_mask(src_pixel),
											   src_alpha_factor(src, dest))),
								divisor_factor(
									multiplier(odd_mask(dest_pixel),
											   dest_alpha_factor(src, dest)))));
					const aligned_wideword_type res_pixel2 =
						even_shifter(
							compute_factor(
								divisor_factor(
									multiplier(even_mask(src_pixel),
											   src_alpha_factor(src, dest))),
								divisor_factor(
									multiplier(even_mask(dest_pixel),
											   dest_alpha_factor(src, dest)))));
